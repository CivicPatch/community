...
// connecting state, and the mute/block menu. Light rows (rest of the room) are
...
const statusCell = (p: Player, onShowStatus: RosterDeps['onShowStatus']) => {
  ...
  if (s.length <= STATUS_PREVIEW)
    return html`<span class="cr-roster-saying" title=${s}>${s}</span>`
  return html`<span class="cr-roster-saying" title=${s}
    >${s.slice(0, STATUS_PREVIEW).trimEnd()}…<button
      class="cr-roster-more"
...
export const makeRosterRow =
  ...
  (p: Player, full: boolean) => {
    ...
    // green ring mirrors the room token: huddle peers ring once actually connected,
    // room people ring when their mic is enabled (presence-synced).
    const ringOn = full ? connected : !!p.audioEnabled
    return html`<li class="cr-roster-item ${isBlocked ? 'cr-blocked' : ''}">
      <span class="cr-roster-avatar ${ringOn ? 'cr-ring' : ''}" aria-hidden="true">${p.avatar || '●'}</span>
      <span class="cr-roster-name">${p.name}</span>
      ${p.status ? statusCell(p, deps.onShowStatus) : ''}
      <span class="cr-visually-hidden">
        ${isBlocked
          ? 'blocked'
          : full
            ? connected
      ...
      ${full
        ? html`
            <button
              class="cr-btn cr-roster-menu-btn"
              aria-haspopup="menu"
            ...
            ${menuOpen
              ? html`<div class="cr-roster-menu" role="menu">
                  ${isBlocked
                    ? html`<button class="cr-btn" role="menuitem" @click=${act(() => deps.toggleBlockPeer(p.id))}>
                        Unblock
                      </button>`
                    : html`<button class="cr-btn" role="menuitem" @click=${act(() => deps.toggleMutePeer(p.id))}>
                          ${isMuted ? 'Unmute' : 'Mute'}
                        </button>
                        <button class="cr-btn" role="menuitem" @click=${act(() => deps.toggleBlockPeer(p.id))}>
                          Block
                        </button>`}
                </div>`
              : ''}
