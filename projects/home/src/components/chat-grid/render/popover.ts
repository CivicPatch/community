// Shared floating popover — the `cg-pop` chrome. One component for the cell description
...
export interface PopoverOpts {
  ...
  below?: boolean
  /** align to an edge instead of centering (near grid edges) */
  align?: 'left' | 'right'
  /** extra modifier classes (e.g. 'cg-pop-raise' to clear a token's name) */
  extra?: string[]
}

export const popover = (opts: PopoverOpts): TemplateResult => {
  const cls = ['cg-pop']
  if (opts.open) cls.push('cg-pop--open')
  if (opts.below) cls.push('cg-pop-below')
  if (opts.align === 'left') cls.push('cg-pop-left')
  else if (opts.align === 'right') cls.push('cg-pop-right')
  if (opts.extra) cls.push(...opts.extra)
  return html`<span class=${cls.join(' ')} aria-hidden="true">
    ${opts.title ? html`<span class="cg-pop-title">${opts.title}</span>` : ''}
    ${opts.body ? html`<span class="cg-pop-body">${opts.body}</span>` : ''}
  </span>`
}
