// The connection lifecycle, run once per config-url: load the grid config, build
// the grid, create the realtime backend + WebRTC mesh + notification pinger, wire
...
import type { Coord, GridConfig, Grid, Player } from '../core/types'
...
import { buildGrid, isWalkable } from '../core/grid'
...
export const useGridConnection = (configUrl: string, deps: GridConnectionDeps) => {
  useEffect(() => {
    let cancelled = false
    const setup = async () => {
      try {
