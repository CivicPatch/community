...
import type { Coord, GridConfig } from './types'
import { buildGrid, coordKey } from './grid'
import { findPath } from './pathfind'

const open = buildGrid({ columns: 3, rows: 3, cells: [] } satisfies GridConfig)
...
describe('findPath', () => {
  ...
  it('routes around a wall', () => {
    ...
    const path = findPath(grid, { col: 0, row: 0 }, { col: 2, row: 0 }, new Set())
    expect(path).not.toBeNull()
    expect(last(path!)).toEqual({ col: 2, row: 0 })
  ...
  it('returns null when the target is walled off', () => {
    ...
    expect(findPath(grid, { col: 0, row: 0 }, { col: 2, row: 0 }, new Set())).toBeNull()
  })
