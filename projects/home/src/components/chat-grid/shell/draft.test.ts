...
const config: GridConfig = { columns: 2, rows: 2, cells: [{ coord: { col: 0, row: 0 }, audio: true }] }
...
describe('draft persistence', () => {
  ...
  it('returns null when the stored payload has no config', () => {
    const s = fakeStorage()
    s._map.set('chat-grid-draft', JSON.stringify({ savedAt: 1 }))
  ...
  it('returns null on malformed JSON', () => {
    const s = fakeStorage()
    s._map.set('chat-grid-draft', '{not json')
    vi.stubGlobal('localStorage', s)
    expect(loadDraft()).toBeNull()
  })
