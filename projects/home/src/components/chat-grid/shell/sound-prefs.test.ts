...
describe('sound preferences', () => {
  ...
  it('fills missing or wrong-typed fields from defaults', () => {
    ...
    s._map.set('chat-grid-sound-prefs', JSON.stringify({ status: true })) // joinLeave missing
    expect(loadSoundPrefs()).toEqual({ joinLeave: true, status: true })
    s._map.set('chat-grid-sound-prefs', JSON.stringify({ joinLeave: 'yes', status: 1 })) // wrong types
    expect(loadSoundPrefs()).toEqual(DEFAULTS)
  })

  it('falls back to defaults on malformed JSON', () => {
    const s = fakeStorage()
    s._map.set('chat-grid-sound-prefs', '{bad')
    vi.stubGlobal('localStorage', s)
    expect(loadSoundPrefs()).toEqual(DEFAULTS)
  })
